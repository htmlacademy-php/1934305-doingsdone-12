...
class ValidatePasswordTest extends TestCase
{
    public function testValidatePassword()
    {
        ...
        $this->assertEquals(
            "Пароль не должен превышать лимит 60-ти символов",
            validatePassword($superLongPassword)
        );
        $this->assertEquals(
            "Пароль не должен содержать пробельные символы",
            validatePassword($passwordWithWhiteSpace)
        );
        $this->assertEquals(
            "Пароль должен содержать минимум 8 символов",
            validatePassword($passwordWithLessEightChars)
        );
        $this->assertEquals(
            "Пароль не должен содержать пробельные символы",
            validatePassword($whiteSpacedPassword)
        );
